...
bmi085x_status_e bmi085a_init(bmi085x *bmi085)
{
	...
	status = bmi085a_write_reg(bmi085->acc.config.meas_range_addr, bmi085->acc.config.meas_range);
	delay_ms(50);
	if(status != BMI085X_SUCCESS)
	{
		return status;
	...
	status = bmi085a_write_reg(bmi085->acc.config.pwr_mode_addr, bmi085->acc.config.pwr_mode);
	if(status != BMI085X_SUCCESS)
	{
		return status;
	}
...
#endif
	bmi085->data.accel_poll_val.x = (( (f32) (CONCAT_RAW_VAL(rx_buff_poll[3], rx_buff_poll[2])) / bmi085a_sensivity) * powf(2.0f, (f32) (bmi085->acc.config.meas_range + 1)));
	bmi085->data.accel_poll_val.y = (( (f32) (CONCAT_RAW_VAL(rx_buff_poll[5], rx_buff_poll[4])) / bmi085a_sensivity) * powf(2.0f, (f32) (bmi085->acc.config.meas_range + 1)));
	bmi085->data.accel_poll_val.z = (( (f32) (CONCAT_RAW_VAL(rx_buff_poll[7], rx_buff_poll[6])) / bmi085a_sensivity) * powf(2.0f, (f32) (bmi085->acc.config.meas_range + 1)));

// Set this in the configuration
#ifdef BMI_DATA_IN_MG
	bmi085->data.accel_poll_val.x *= 1000;
	bmi085->data.accel_poll_val.y *= 1000;
	bmi085->data.accel_poll_val.z *= 1000;
